...
class GameSameGroup:
    ...
    # group: {'players': [User], 'gamename': GameSameGroup name, 'anything': anything}
    uncomplete: 'dict[Group, GameData]' = {}
    center: 'dict[Group, GameData]' = {}
    def __init__(self, name: str, name_zh: str, player: Tuple[int, int],
                 type_gamedata: type[GameData], allow_private: bool=False):
        self.name = name
        ...
        self.GameDataType = type_gamedata
    @classmethod
    def get(cls, name: str):
        return cls.all_games[name]
    ...
    async def get_event_data(self, channel: DiscordGroup=Depends(getGroup)):
        ...
        if data and data.gamename == self.name:
            return data
    @property
    def data(self):
    ...
    async def checkInGroup(self, matcher: Matcher, group: DiscordGroup=Depends(getGroup)):
        if group not in self.center or self.center[group].gamename != self.name:
            matcher.skip()
    ...
    async def checkToBegin(self, group: DiscordGroup, bot: Bot) -> bool:
        ...
        if group not in self.uncomplete:
            return False
        if self.uncomplete[group].gamename != self.name:
            return False
    ...
    def begin_message_handle(self):
        ...
        return deco_handle
    def start(self):
        async def onBegin(bot: Bot, state: T_State, group: DiscordGroup=Depends(getGroup), user: DiscordUser=Depends(getUser)):
            ...
            if group in self.center:
                ...
                matcher.skip()
            self.uncomplete[group] = self.GameDataType(players=[user], gamename=self.name)
        ...
        pml = [Depends(onBegin, use_cache=False),
               ...
               Depends(self.checkBegin)]
        if not self.allow_private:
            pml = [Depends(requireNoDM)] + pml
        f1 = matcher.handle_sub_command(self.name, 'begin',
                parameterless=pml)
        f2 = click.handle([Depends(checkClick),
                           Depends(self.messageHandle),
                           Depends(self.checkBegin)])
...
async def checkClick(bot: Bot, matcher: Matcher, event: MessageComponentInteractionEvent,
    ...
    game: GameSameGroup = GameSameGroup.get(GameSameGroup.uncomplete[group].gamename)
    if button == "attend":
...
class GamePrivate:
    ...
    async def checkInGame(self, matcher: Matcher, user: DiscordUser=Depends(getUser)):
        room = self.get_event_data(user)
        if room is None or not room.begin or room.gamename != self.name:
            matcher.skip()
    ...
    def open(self):
        async def onOpen(type: CommandOption[str], password: CommandOption[str],
            ...
            if user in self.players_status:
                ...
                room = self.center[room_id] = TRoomPrivate(
                    players = [user], public = not private, type = type,
                    gamename = self.name, id = room_id, password = password,
