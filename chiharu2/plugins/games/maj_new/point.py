...
class AllCheckers:
    ...
    Yisebubugao:            PaixingRanker = \
        lambda s, c, p: 4 if len(l := more_itertools.one((l2 := list(pai.num for pai in b[cl])) for cl in Color if 
                            len(list((b := more_itertools.bucket(sorted(set(s.shunziMinList())), lambda pai: pai.color))[cl])) >= 3)) == 4 \
                            and l
