    ...
    if command.startswith("帮助"):
        ...
        async def proceed_buy(buying: Callable[[], Literal[2, 3, 4, 5]], this: int):
            try:
                ...
                player.state = ret
                if ret == 2:
                    await matcher.send("请模仿你已有的颜色！" if this == 2 else "请选择模仿卡所模仿的颜色。")
                elif ret == 3:
                    await matcher.send("请偷取对手有的颜色！" if this == 3 else "请选择偷取的宝石颜色。")
                elif ret == 4:
                    await matcher.send("请从板中正确拿取对应颜色的宝石！" if this == 4 else "请选择拿取宝石的位置。")
                elif ret == 5:
                    await matcher.send("请正确拿取已有的皇冠卡！" if this == 5 else "请选择拿取的皇冠卡。")
            except StopIteration as e:
                rete: Literal[-1, -3, -2, 1] = e.value
                if rete == -1:
                    await matcher.send("购买失败！")
                elif rete == -2:
                    await matcher.send("货币不足！")
                elif rete == -3:
                    await matcher.send("不可先购买模仿卡！")
                else:
                    player.state = 1
        ...
        if player.state == 0:
            if command.startswith("使用特权") or command.startswith("使用卷轴"):
                ...
                if len(lm) == 0:
                    await matcher.send("请正确输入宝石坐标！")
                    return
                l = [GemCorr(x) for x in lm]
                if (-1, -1) in l:
                    await matcher.send("请输入正确的宝石坐标！")
                    return
                ret = player.UseScroll(l)
                if ret == -1:
                    await matcher.send("卷轴数量不够！")
                elif ret == -2:
                ...
                if len(lm) == 0 or len(lm) > 3:
                    await matcher.send("请正确输入至多三个宝石坐标！")
                    return
                l = [GemCorr(x) for x in lm]
                if (-1, -1) in l:
                    await matcher.send("请输入正确的宝石坐标！")
                ...
                if ret == -1:
                    await matcher.send("请拿在同一条直线上的三个宝石！")
                elif ret == 2:
                    await matcher.send("对手已拿取一个卷轴。")
                    player.state = 1
                else:
                    player.state = 1
