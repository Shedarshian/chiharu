...
from ..helper.helper import DiscordGroup, DiscordUser, getUser, getGroup, rel
...
class AI(Player):
    def process(self) -> tuple[str, list[int]]:
        with open(rel(f"yahtzeeAI\\exp{12 - len(self.scoreboard)}-{self.rolled_count}.csv"), encoding="utf-8") as f:
            els = ""
            saved_count = 0
            for line in f:
        ...
        if self.rolled_count == 0:
            ...
            with open(rel(f"yahtzeeAI\\exp{12 - len(self.scoreboard)}-0.csv"), encoding="utf-8") as f:
                for line in f:
                    stat, wjscore, _, els = line.split(",", 3)
                    if stat == ''.join("1" if name in self.scoreboard else "0" for name in reversed(Player.Name)) and int(wjscore) in (-1, self.wjscore):
                        break
...
# Required to all have default parameters
@dataclass(config=ConfigDict(arbitrary_types_allowed=True))
class YahtzeeGameData(GameData):
    boards: list[Player] = field(default_factory=list)
...
yahtzee = GameSameGroup('yahtzee', '快艇骰子', (1, 4), YahtzeeGameData)
...
@yahtzee.start()
async def yahtzee_begin(matcher: Matcher,
                        data: YahtzeeGameData=yahtzee.data):
    # data: {'players': [qq], 'game': GameSameGroup instance, 'args': [args], 'anything': anything}
    await matcher.send('游戏开始，任何时候输入"查看分数"可以查看全部玩家当前分数')
    # await bot.send_to(group.channel_id, '游戏开始，任何时候输入"查看分数"可以查看全部玩家当前分数')
    #开始游戏
    data.boards = [Player() for qq in data.players]
    data.current_player = 0
    p = data.boards[0]
    p.roll()
    await matcher.send(f'玩家{data.current_name}扔出骰子{p.float_dice}，已固定骰子{p.fixed_dice}\n剩余重扔次数：{p.rolled_count}\n输入如"重扔 5,5,6"重扔，如"计分 快艇"计分')

@yahtzee.process()
async def yahtzee_process(matcher: Matcher,
                          data: YahtzeeGameData=yahtzee.data,
                          delete_func: DeleteFunc=yahtzee.delete_func,
    ...
    if command.startswith('重扔') and user == data.players[data.current_player]:
        if p.rolled_count <= 0:
            await matcher.send("您没有重扔次数！")
            return
        try:
            if p.unfix([int(c) for c in command[2:].strip().replace('，', ',').split(',')]) == -1:
                await matcher.send('未发现骰子，请重新输入')
                return
        except ValueError:
            await matcher.send('未发现骰子，请重新输入')
            return
        p.roll()
        if p.rolled_count == 0:
            await matcher.send(f'玩家{data.current_name}骰子为{p.fixed_dice}\n可选计分项为：{p.str_temp_scoreboard}，\n输入如"计分 快艇"计分')
        else:
            await matcher.send(f'玩家{data.current_name}扔出骰子{p.float_dice}，已固定骰子{p.fixed_dice}\n剩余重扔次数：{p.rolled_count}\n输入如"重扔 5,5,6"重扔，如"计分 快艇"计分')
    elif command.startswith('计分') and user == data.players[data.current_player]:
        c = command[2:].strip()
        if c not in Player.Name.__members__:
            await matcher.send('未发现此计分项，请重新输入')
            return
        ret = p.score(Player.Name.__members__[c])
        if ret == -1:
            await matcher.send(f'计分项{c}已计过，请重新输入')
            return
        await matcher.send(f'玩家{data.current_name}计分 {c}，{ret}点。')
        data.current_player += 1
        if data.current_player >= len(data.players):
            data.current_player = 0
            if (ai := data.ai) is not None:
                ai.roll()
                for _ in range(3):
                    if ai.rolled_count == 0:
                        await matcher.send(f'AI骰子为{ai.fixed_dice}\n剩余重扔次数：0')
                    else:
                        await matcher.send(f'AI扔出骰子{ai.float_dice}，已固定骰子{ai.fixed_dice}，总骰子{ai.all_dice}\n剩余重扔次数：{ai.rolled_count}')
                    await sleep(4)
                    do, which = ai.process()
                    if do == "计分":
                        choose = Player.Name(which[0])
                        ai.score(choose)
                        await matcher.send("AI计分" + choose.name + "，当前得分：\n" + ai.str_scoreboard)
                        await sleep(1)
                        break
                    else:
                        await matcher.send("AI重扔" + ",".join(str(i) for i in which) + "。")
                        await sleep(1)
                        ai.unfix(which)
                        ai.roll()
            if len(data.boards[data.current_player].scoreboard) == 12:
                await matcher.send('游戏结束，计分板如下：\n' + '\n'.join(f'玩家{player.name}分数：\n{board.str_scoreboard}\n总分为：{board.final_score}分' for (player, board) in zip(data.players, data.boards)) + (f'\nAI分数：\n{data.ai.str_scoreboard}\n总分为：{data.ai.final_score}分' if data.ai is not None else ""))
                f = [board.final_score for board in data.boards]
                if data.ai is not None:
                    f.append(data.ai.final_score)
                m = max(f)
                await matcher.send('玩家' + '，'.join([([x.name for x in data.players] + ["AI"])[i] for i, x in enumerate(f) if x == m]) + '胜出！')
                if m > 200:
                    for i, x in enumerate(f):
                        if i != len(f) - 1 and x == m:
                            if achievement.yahtzee.get(str(data.players[i])):
                                await matcher.send(achievement.yahtzee.get_str())
                        if i == len(f) - 1 and x == m:
        ...
        p.roll()
        await matcher.send(f'轮到玩家{data.current_name}，扔出骰子{p.float_dice}，已固定骰子{p.fixed_dice}\n剩余重扔次数：{p.rolled_count}\n输入如"重扔 5,5,6"重扔，如"计分 快艇"计分')
    elif command == '查看分数' or command == '查询分数':
        await matcher.send('\n'.join(f'玩家{player.name}分数：\n{board.str_scoreboard}' for (player, board) in zip(data.players, data.boards)) + (f"\nAI分数：\n{data.ai.str_scoreboard}" if data.ai is not None else ""))
    elif command == '重新查询':
        await matcher.send(f'您当前扔出骰子{p.float_dice}，已固定骰子{p.fixed_dice}\n剩余重扔次数：{p.rolled_count}\n输入如"重扔 5,5,6"重扔，如"计分 快艇"计分')
...
#         yahtzee.uncomplete[group_id].ai = AI()
