...
class SnakeBird:
    ...
    def cal_food(self):
        ...
        self.portal_activate = False if len(self.portal) == 0 else self[self.portal[0]] == '0' and self[self.portal[1]] == '0'
    @methdispatch
    ...
    def image(self):
        ...
        # next 123 in board
        d = {(-1, 0): [5, 0, 10, 2], (0, 1): [11, 5, 15, 10], (1, 0): [5, 11, 10, 15], (0, -1): [0, 5, 2, 10]}
        ...
        for i, line in enumerate(self.board):
            for j, block in enumerate(line):
                if block == '1':
                    ...
                    draw.polygon([16 * j + 7, 16 * i + 3, 16 * j + 4, 16 * i + 11, 16 *j + 11, 16 * i + 11], self.color['2'])
                elif block == '3':
                    pos = (i, j)
                    _in = [dir for dir in self.dir if in_border(add(pos, dir), self.border) and self[add(pos, dir)] == '3']
                    if len(_in) == 0:
                        crop = (0, 0, 16, 16)
                        trans = None
                    elif len(_in) == 1:
                        crop = (16, 0, 32, 16)
                        trans = {(-1, 0): Image.ROTATE_180, (1, 0): None, (0, 1): Image.ROTATE_90, (0, -1): Image.ROTATE_270}[_in[0]]
                    elif len(_in) == 2 and add(*_in) == (0, 0):
                        crop = (0, 16, 16, 32)
                        trans = [None, Image.ROTATE_90][abs(_in[0][0])]
                    elif len(_in) == 2:
                        crop = (32, 0, 48, 16)
                        trans = {(-1, 1): Image.ROTATE_90, (-1, -1): Image.ROTATE_180, (1, -1): Image.ROTATE_270, (1, 1): None}[add(*_in)]
                    elif len(_in) == 3:
                        crop = (16, 16, 32, 32)
                        trans = {(-1, 0): Image.ROTATE_180, (1, 0): None, (0, 1): Image.ROTATE_90, (0, -1): Image.ROTATE_270}[(set(self.dir) - set(_in)).pop()]
                    else:
                        crop = (32, 16, 48, 32)
                        trans = None
                    c = f.crop(crop)
        ...
        for id, body in self.snake.items():
            ...
            for i_b, pos in enumerate(b):
                ...
                if i_b != 0:
                    ...
                    # draw eye on head
                    if self.activate == id:
                        draw.arc([16 * j + 5, 16 * i + 5, 16 * j + 10, 16 * i + 10], 0, 360, (255, 255, 255), 10)
                        draw.rectangle([16 * j + 7, 16 * i + 7, 16 * j + 7, 16 * i + 7], (0, 0, 0))
                    else:
                        draw.rectangle([16 * j + 5, 16 * i + 7, 16 * j + 9, 16 * i + 7], (0, 0, 0))
                if i_b != len(b) - 1:
                    draw.rectangle(list(
                        itertools.starmap(lambda x, y: x + y,
                            zip([16 * j, 16 * i, 16 * j, 16 * i],
...
@on_command(('sb'), only_to_me=False)
@config.ErrorHandle
async def sbegin(session: CommandSession):
    global board
    board = SnakeBird(session.current_arg_text)
